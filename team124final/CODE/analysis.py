...
def analysis(param: str, all_years: bool):
    ...
    input_file_path = '.\\CODE\\Analytic File 3-31-21 DIB.csv'
    # input_file_path = '../Analytic File 3-31-21 DIB.csv' # mac path
    ...
    for st in states:
        if all_years:
            print(f'evaluating state {st} for all years for parameter {op_analysis.param}')
            run_analysis(df=full_df, state=st)
        else:
            # for each year
            for yr in year:
                print(f'evaluating state {st} in year {yr} for parameter {op_analysis.param}')
                # filter to single year
                yr_df = full_df[full_df['YR'] == yr]
                run_analysis(df=yr_df, state=st)                                        
    
    # write to csv     
    op_analysis.output_results(path='.\CODE\\')
